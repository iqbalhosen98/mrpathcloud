...
<html lang="{{ config('app.locale') }}">
    <head>
        ...
        @else
            <link rel="icon" sizes="16x16" href="{{ asset('vendor/mrpath/ui/assets/images/favicon.ico') }}" />
        @endif

        <link rel="stylesheet" href="{{ asset('vendor/mrpath/ui/assets/css/ui.css') }}">
        <link rel="stylesheet" href="{{ asset('vendor/mrpath/admin/assets/css/admin.css') }}">

        @yield('head')

        @stack('css')

        {!! view_render_event('mrpath.admin.layout.head') !!}
    </head>

    <body @if (core()->getCurrentLocale() && core()->getCurrentLocale()->direction == 'rtl') class="rtl" @endif style="scroll-behavior: smooth;">

        {!! view_render_event('mrpath.admin.layout.body.before') !!}

        <div id="app">
            <flash-wrapper ref='flashes'></flash-wrapper>

            {!! view_render_event('mrpath.admin.layout.nav-top.before') !!}

            @include ('admin::layouts.nav-top')

            {!! view_render_event('mrpath.admin.layout.nav-top.after') !!}

            {!! view_render_event('mrpath.admin.layout.nav-left.before') !!}

            @include ('admin::layouts.nav-left')

            {!! view_render_event('mrpath.admin.layout.nav-left.after') !!}

            <div class="content-container" :class="isMenuOpen ? 'padding-container-navbar-expand' : 'padding-container-navbar-not-expand'">

                {!! view_render_event('mrpath.admin.layout.content.before') !!}

                @yield('content-wrapper')

                {!! view_render_event('mrpath.admin.layout.content.after') !!}
        ...
        <script type="text/javascript" src="{{ asset('vendor/mrpath/admin/assets/js/admin.js') }}"></script>

        <script type="text/javascript" src="{{ asset('vendor/mrpath/ui/assets/js/ui.js') }}"></script>
        ...
        {!! view_render_event('mrpath.admin.layout.body.after') !!}
