...
@section('content')
    <div class="content">
        ...
        {!! view_render_event('mrpath.admin.catalog.product.edit.before', ['product' => $product]) !!}
            ...
            <div class="page-content">
                ...
                @foreach ($product->attribute_family->attribute_groups as $index => $attributeGroup)
                    ...
                    @if (count($customAttributes))
                        ...
                        {!! view_render_event('mrpath.admin.catalog.product.edit_form_accordian.' . $attributeGroup->name . '.before', ['product' => $product]) !!}

                        <accordian :title="'{{ __($attributeGroup->name) }}'"
                                   :active="{{$index == 0 ? 'true' : 'false'}}">
                            <div slot="body">
                                {!! view_render_event('mrpath.admin.catalog.product.edit_form_accordian.' . $attributeGroup->name . '.controls.before', ['product' => $product]) !!}
                                ...
                                {!! view_render_event('mrpath.admin.catalog.product.edit_form_accordian.' . $attributeGroup->name . '.controls.after', ['product' => $product]) !!}
                            </div>
                        </accordian>

                        {!! view_render_event('mrpath.admin.catalog.product.edit_form_accordian.' . $attributeGroup->name . '.after', ['product' => $product]) !!}
                ...
                {!! view_render_event(
                  'mrpath.admin.catalog.product.edit_form_accordian.additional_views.before',
                   ['product' => $product])
                !!}
                ...
                {!! view_render_event(
                  'mrpath.admin.catalog.product.edit_form_accordian.additional_views.after',
                   ['product' => $product])
                !!}
            </div>
        ...
        {!! view_render_event('mrpath.admin.catalog.product.edit.after', ['product' => $product]) !!}
    </div>
@stop
        ...
        $(document).ready(function () {
            $('#channel-switcher, #locale-switcher').on('change', function (e) {
                ...
                if (event.target.id == 'channel-switcher') {
                    let locale = "{{ app('Mrpath\Core\Repositories\ChannelRepository')->findOneByField('code', $channel)->locales->first()->code }}";
