@php
    $attributeRepository = app('\Mrpath\Attribute\Repositories\AttributeFamilyRepository');
    $comparableAttributes = $attributeRepository->getComparableAttributesBelongsToFamily();

    $locale = core()->getRequestedLocaleCode();

    $attributeOptionTranslations = app('\Mrpath\Attribute\Repositories\AttributeOptionTranslationRepository')->where('locale', $locale)->get();
@endphp
...
@push('scripts')
    <script type="text/x-template" id="compare-product-template">
        <section class="cart-details row no-margin col-12">
            ...
            {!! view_render_event('mrpath.shop.customers.account.compare.view.before') !!}
                                    ...
                                    @case('product_image')
                                        <a :href="`${$root.baseUrl}/${product.url_key}`" class="unset">
                                            <img
                                                ...
                                                :onerror="`this.src='${$root.baseUrl}/vendor/mrpath/ui/assets/images/product/large-product-placeholder.png'`"
                                                alt=""/>
                                        </a>
                                        @break
            ...
            {!! view_render_event('mrpath.shop.customers.account.compare.view.after') !!}
        </section>
    </script>
