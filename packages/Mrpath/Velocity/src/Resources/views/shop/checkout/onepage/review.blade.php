                ...
                @foreach ($cart->items as $item)
                    ...
                    <div class="row col-12 no-padding">
                        ...
                        <div class="col-10 no-padding fs16">
                            ...
                            {!! view_render_event('mrpath.shop.checkout.name.before', ['item' => $item]) !!}

                                <div class="row fs20">
                                    <span class="col-12 link-color fw6">{{ $item->product->name }}</span>
                                </div>

                            {!! view_render_event('mrpath.shop.checkout.name.after', ['item' => $item]) !!}

                            <div class="row col-12">
                                {!! view_render_event('mrpath.shop.checkout.price.before', ['item' => $item]) !!}
                                        <span class="value">
                                            {{ core()->currency($item->base_price) }}
                                        </span>
                                {!! view_render_event('mrpath.shop.checkout.price.after', ['item' => $item]) !!}

                                <i class="rango-close text-down-4"></i>

                                {!! view_render_event('mrpath.shop.checkout.quantity.before', ['item' => $item]) !!}
                                    <span class="value">
                                        {{ $item->quantity }} ({{ __('shop::app.checkout.onepage.quantity') }})
                                    </span>
                                {!! view_render_event('mrpath.shop.checkout.quantity.after', ['item' => $item]) !!}
                            </div>
                            ...
                            {!! view_render_event('mrpath.shop.checkout.options.before', ['item' => $item]) !!}
                            ...
                            {!! view_render_event('mrpath.shop.checkout.options.after', ['item' => $item]) !!}
                        </div>
                    </div>
                @endforeach
            </div>
