            ...
            <div class="dropdown-cart">
                <div class="dropdown-header">
                    <p class="heading">
                        ...
                        {!! view_render_event('mrpath.shop.checkout.cart-mini.subtotal.before', ['cart' => $cart]) !!}

                        @if (Mrpath\Tax\Helpers\Tax::isTaxInclusive())
                            <b>{{ core()->currency($cart->base_grand_total) }}</b>
                        @else
                            <b>{{ core()->currency($cart->base_sub_total) }}</b>
                        @endif

                        {!! view_render_event('mrpath.shop.checkout.cart-mini.subtotal.after', ['cart' => $cart]) !!}
                    </p>
                </div>
                    ...
                    @foreach ($items as $item)
                        ...
                        <div class="item">
                            ...
                            <div class="item-details">
                                {!! view_render_event('mrpath.shop.checkout.cart-mini.item.name.before', ['item' => $item]) !!}

                                <div class="item-name">{{ $item->name }}</div>

                                {!! view_render_event('mrpath.shop.checkout.cart-mini.item.name.after', ['item' => $item]) !!}


                                {!! view_render_event('mrpath.shop.checkout.cart-mini.item.options.before', ['item' => $item]) !!}
                                ...
                                {!! view_render_event('mrpath.shop.checkout.cart-mini.item.options.after', ['item' => $item]) !!}


                                {!! view_render_event('mrpath.shop.checkout.cart-mini.item.price.before', ['item' => $item]) !!}
                                ...
                                {!! view_render_event('mrpath.shop.checkout.cart-mini.item.price.after', ['item' => $item]) !!}


                                {!! view_render_event('mrpath.shop.checkout.cart-mini.item.quantity.before', ['item' => $item]) !!}

                                <div class="item-qty">Quantity - {{ $item->quantity }}</div>

                                {!! view_render_event('mrpath.shop.checkout.cart-mini.item.quantity.after', ['item' => $item]) !!}
                            </div>
                        </div>
